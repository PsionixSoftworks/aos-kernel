...
gdt_t gdt_entries[MAX_GDT_ENTRIES];
pgdt_t pgdt;
...
static inline void gdt_set_gate(dword index, udword base, udword limit, ubyte access, ubyte granularity);
...
inline void
gdt_add_descriptor(init_desc descriptor, udword base, udword limit, ubyte access, ubyte granularity)
{
	dword i = 0;
	descriptor = &gdt_set_gate;

	descriptor(i++, base, limit, access, granularity);
...
tss_write(uint32_t num)
{
	...
	tss_entry.esp0 = REPLACE_KERNEL_STACK_ADDR;
