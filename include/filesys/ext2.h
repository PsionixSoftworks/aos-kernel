...
typedef struct 
{
    ...
    uint32_t last_mount;
    uint32_t last_write;
    uint16_t mount_since_last_check;
...
typedef struct 
{
    ...
    uint16_t num_of_unallocated_block;
    uint16_t num_of_unallocated_inode;
    uint16_t num_of_usable_inode;
...
extern uint8_t ext2_probe(device_t *device);
extern uint8_t ext2_mount(device_t *device, void *priv);
